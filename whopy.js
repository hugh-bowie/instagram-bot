...
const { r, log, logD, device, badAccounts, r15, r23, logW } = require('./src/helpers');
...
(async () => {
	try {
		...
		//---- go to vibe_szn
		await page.goto('https://www.instagram.com/vibe_szn', { waitUntil: 'networkidle2' });
		const user = await page.$eval('h1.K3Sf1', use => use.innerText);
		const flws = await page.$$eval('a[href$="/followers/"]', flw => flw.map(fl => fl.children[0].children[0].innerText));
		logD(`${user} Flwrs:${flws}`);
		log(`\n${user} Flwrs:${flws}`);

		const flwsHndl = await page.$('a[href$="/followers/"]');
		if (flwsHndl) {
			await Promise.all([page.waitForNavigation({ waitUntil: 'networkidle2' }), page.tap('a[href$="/followers/"] div')]);
			await page.waitForTimeout(r15);
			await page.waitForSelector('h1', { visible: true });
		}


		let flwsH1 = await page.$x('//h1[contains(text(), "Followers")]');
		if (flwsH1) {
			for (let i = 0; i < 50; i++) {
		...
		let publicFlws = await page.$$eval('div[aria-disabled="false"]', pub => pub.map(pu => pu.nextSibling.children[0].children[0].getAttribute('href')));
		let privateFlws = await page.$$eval('div[aria-disabled="true"]', pri => pri.map(pr => pr.nextSibling.children[0].children[0].getAttribute('href')));
		if (publicFlws) {
			await page.waitForTimeout(r15);
			logW(`Found ${publicFlws.length} Public accounts\n\n `);

			for (let f = 0; f < publicFlws.length; f++) {
				await page.waitForTimeout(100);
				logW(`#${f} https://www.instagram.com${publicFlws[f]}\n`);
			}

		}


		// logW(`Found ${privateFlws.length} Private accounts\n\n`);
		// for (let f = 0; f < privateFlws.length; f++) {
		// 	logW(`#${f}  ${privateFlws}\n`);
		// }
